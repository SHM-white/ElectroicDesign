...
运行器构建 Livox 和 FAST-LIO 的 ROS 2 版本，使用 `localization_mode:=fast_lio` 启动 `ed_uav_gazebo gazebo_simulation.launch.py`、Gazebo GUI 和 RViz，并等待 LIO、地图、规划器、FCU 和任务接口就绪。它要求定位状态为 ACTIVE 且存在有效的 `map -> odom` 变换，然后才通过 `/fcu/flight_command` 解锁，向 `/mission/execute` 发送 `simulation-competition`。它会记录成功的动作结果，并确认模拟器 FCU 在本次运行成功前电机已锁定。
