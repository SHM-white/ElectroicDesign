...
- 位置状态行：`x=24 y=288 w=752 h=32`、`gs_type_body`，必要时重复文字状态并
  使用对应的语义颜色。
- Detail 按钮：`x=680 y=424 w=96 h=48`，文字为 `DETAIL`。
...
- Title label: `DETAIL`, `x=24 y=8 w=320 h=48`, `gs_type_title`,
  `gs_color_masthead_text`.
- 位置时长行：`x=24 y=88 w=752 h=72`。标签为 `x=24 y=88 w=752 h=24`。
  值为 `x=24 y=112 w=752 h=48`、`gs_type_value`。渲染
...
- 质量行：`x=24 y=296 w=752 h=72`。标签为 `x=24 y=296 w=752 h=24`。
  值为 `x=24 y=320 w=752 h=48`、`gs_type_value`。无 `quality` 时渲染 `QUALITY UNKNOWN`，
...
- 仅在按钮释放时导航：`DETAIL` 从 Overview 进入 Detail，`BACK` 从 Detail 返回
  Overview。按下、长按、拖动和未知目标均不执行操作。
...
- 对比度：根据精确界面验证主要文字和值；深色页眉上的文字只能使用定义的白色令牌，
  绝不能仅通过颜色传达状态。
...
- 接受的欠账仅限 Waveshare 板上的物理显示、触摸和阳光验证。在此欠账下不推迟任何
  软件行为或设计功能。
