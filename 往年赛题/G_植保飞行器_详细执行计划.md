...
> **2026-07 当前实现基线**：树莓派通过 USB-TTL 以 500000bps 直连凌霄
> IMU 串口2，直接收发匿名通信协议 V7 原生帧。早期文档中的 STM32F4
> `0xAA/0xBB/0xCC` 桥接、主动查询和自定义心跳仅作为历史方案保留，当前
> 程序不使用。水平连续位置以 V7 `0x08` 为主；`0x51` 不得与 `0x08`
> 混写为同一连续坐标源。工业/MVS相机承担A点、数字OCR、返航十字和灰色
> 数字中心检测；导航约完成 2/3 后使用灰色中心执行有界辅助校准。
...
当前 `drone/mcu_serial.py` 不经过自定义 MCU 桥接，帧格式统一为：
...
- 目标地址通常为 `0xFF`；
- 指令使用 `ID=0xE0`；
- 飞控主动输出高度、模式、位置、电压和遥控通道；
- 水平移动为 `CID=0x10, CMD0=0x02, CMD1=0x03`；
- `CMD2~3` 为距离cm，`CMD4~5` 为速度cm/s，`CMD6~7` 为相对当前机头
    顺时针方向角；
- `0x08 POS_X/POS_Y` 表示相对起飞点的位置偏移，单位cm，是树莓派导航
    粗定位的主要遥测；
- `0x51` 为光流模块信息，只能作为独立诊断或备用来源，切换来源时必须
    重建基准，禁止直接计算跨来源增量。

本章后续 `0xAA/0xBB/0xCC` 自定义桥接格式属于早期固件设计，不再作为
当前实现和场测依据。
...
当前支持 `industrial`、`mvs` 和 `openmv` 后端。MVS/工业相机在树莓派上
执行OpenCV与异步OCR；OpenMV只返回绿色占比和编号，不提供灰色中心，因此
导航灰色中心辅助校准仅在能够返回原始图像的后端启用。

新增 `drone/gray_marker.py`：不识别具体编号，而是在绿色背景中提取低饱和
中性灰色字符，按字符高度、基线和间隔合并一至两个字符，并输出整体外接框
中心、置信度、暗场标志和检测框。该观测必须绑定当前导航目标，不能单独用于
确定区块身份。

场测预览框显示：当前状态、目标区块、路线进度、剩余距离、绿色占比、OCR、
灰色中心框和十字、置信度、前/右厘米误差、校准次数、当前容差、最后校准
动作或降级原因。
...
1. **V7 `0x08` 光流/惯导位置**：20Hz主循环中连续累计，负责粗导航；
2. **绿色占比跳变**：只作为边界观测和日志，不再直接推进路径或复位光流；
3. **数字OCR/A/返航十字**：提供区块身份或起降点绝对确认；
4. **灰色数字中心**：不识别编号，在已知目标上下文中提供局部几何偏差。

每段 `NAVIGATE` 首次发送路线命令时冻结起点 $S$、目标 $T$ 和初始距离。
当前位置 $P$ 的路线进度为：

$$
p=\operatorname{clamp}\left(\frac{(P-S)\cdot(T-S)}{\|T-S\|^2},0,1\right)
$$

当 $p\ge0.66$ 时启用灰色中心检测。连续2~3个独立帧满足置信度和稳定度
后，结合当前高度、相机内参和相机到作业点安装偏移，将像素误差转换为前/右
厘米误差。首次校准采用约7cm容差和最大15cm步长；第二次及以后放宽到约
11cm容差并限制为10cm步长。每个区块最多3次修正、累计约35cm、总计约6秒。

灰色中心缺失、不稳定、误差不下降、达到次数/距离/时间上限时，记录
`degraded` 或 `skipped` 并继续路线，绝不永久阻塞任务。灰色中心只负责
几何校准，OCR仍负责确认当前目标编号。
...
全局 `FlightState` 不变，`NAVIGATE` 内增加：

```text
ROUTE → ACQUIRE_GRAY → WAIT_CORRECTION → ROUTE/ARRIVAL
```

- `ROUTE`：发送并跟踪主路线命令；
- `ACQUIRE_GRAY`：路线完成约2/3后采集稳定灰色中心；
- `WAIT_CORRECTION`：发送一次有限步长修正后等待，不逐帧重复发命令；
- `ARRIVAL`：光流到达且校准已完成、跳过或降级后进入播撒。

人工场测中同样输出修正建议/模拟指令。首次对准严格，二次及以后提高容错；
超时或达到限制后自动继续现有OCR/人工超时流程，避免操作者未对准导致卡死。
...
1. 使用 `python -m drone.test_gray_centers` 回放保存图片，确认框和中心正确；
2. 使用 `drone/field_test.sh` 的模拟飞控+真实MVS模式，观察进度达到66%后才
    启用灰色检测；
3. 检查预览详情框中的中心、置信度、前/右误差、容差和建议动作；
4. 故意不对准，确认首次严格、二次放宽，最终会降级继续而非卡死；
5. 无桨确认凌霄收到新平移命令时是覆盖、排队还是叠加；未确认前可关闭
    `gray_calibration_auto_command`，仅保留建议和日志；
6. 最后以低速短路径对比仅光流与灰色中心辅助后的终点误差。

日志事件统一使用 `[NAV]` 和 `[GRAY_CAL]`，包含目标区块、路线进度、中心框、
置信度、误差、容差、修正次数、距离/方向、真实/模拟来源以及完成、跳过、
超时或降级原因。
