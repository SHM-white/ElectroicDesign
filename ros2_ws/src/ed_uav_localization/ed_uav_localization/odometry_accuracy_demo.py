"""Measure relative odometry behavior from a bounded ROS 2 subscription."""
...
class OdometryAccuracyTrial:
    """Mutable receiver state whose samples arrive through ROS callbacks."""
    ...
    def _receive_sample(self, sample: OdometrySample) -> None:
        ...
        try:
            evaluation = evaluate_odometry_accuracy(
                ...
                self._configuration.known_distance_m,
            )
        except OdometryValidationError as error:
            self._reject(error.issue.value)
            return
        self._samples.append(sample)
        elapsed_sec = (sample.stamp_ns - self._samples[0].stamp_ns) * 1e-9
        if elapsed_sec >= self._configuration.duration_sec:
