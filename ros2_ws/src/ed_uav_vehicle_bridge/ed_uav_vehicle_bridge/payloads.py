...
TELEMETRY_FIXED: Final = struct.Struct(">HBBffBB")
...
def encode_vehicle_telemetry(value: VehicleTelemetryValue) -> bytes:
    ...
    flags = int(value.start_event) | (int(value.heartbeat_alive) << 1) | (int(value.lap_complete) << 2)
...
def decode_vehicle_telemetry(payload: bytes) -> VehicleTelemetryValue:
    ...
    version, flags, raw_motion, displacement, speed, raw_turn, raw_route = reader.unpack(TELEMETRY_FIXED)
...
def decode_mission_selection(payload: bytes) -> MissionSelectionValue:
    ...
    if selection_id == 0 or car_epoch == 0:
        raise ProtocolError(ProtocolErrorCode.BAD_PAYLOAD, "selection ID and car epoch must be nonzero")
    try:
        task = DTask(raw_task)
    except ValueError as error:
...
def _require_contract_version(version: int) -> None:
    if version != 1:
        raise ProtocolError(ProtocolErrorCode.BAD_PAYLOAD, f"unsupported contract version {version}")
...
def _encode_string(value: str, maximum_bytes: int) -> bytes:
    try:
        ...
        raise ProtocolError(ProtocolErrorCode.BAD_PAYLOAD, "string is not valid UTF-8") from error
    if not encoded or len(encoded) > maximum_bytes:
        raise ProtocolError(ProtocolErrorCode.BAD_PAYLOAD, f"string must contain 1-{maximum_bytes} bytes")
