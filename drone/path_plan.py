...
_LAYOUT: Dict[Tuple[int, int], int] = {
    # (row, col): block_id
    (0, 0): 28, (0, 1): 27, (0, 2): 26, (0, 3): 25,
    (0, 4): 24, (0, 5): 23, (0, 6): 22,
    (1, 0): 21, (1, 1): 20, (1, 2): 19, (1, 3): 18,
    (1, 4): 17, (1, 5): 16, (1, 6): 15,
    (2, 3): 14, (2, 4): 13, (2, 5): 12, (2, 6): 11,
    (3, 3): 10, (3, 4): 9,
    (4, 3): 8, (4, 4): 7, (4, 5): 6, (4, 6): 5,
    (5, 3): 4, (5, 4): 3, (5, 5): 2, (5, 6): 1,
}

# 总区块数
TOTAL_BLOCKS = len(_LAYOUT)  # = 28

# 区块中心相对起降点的偏移 (cm): X=机头前方, Y=机体右侧
ORIGIN_OFFSET_X = 0
ORIGIN_OFFSET_Y = 50
...
# 路径策略:
# 1. 从A点21沿第二行移动到18
# 2. 从18沿Col3向下到底, 再沿Col4向上到17/16
# 3. 从16沿Col5向下到底, 再沿Col6向上到22
# 4. 由22向左遍历最上行到28
# 5. 终点28与A点21相邻, 便于从地图左侧返航
PATH: List[int] = [
    # 第二行: A点向右到18
    21, 20, 19, 18,

    # 中间两列: Col3向下, Col4向上到17/16
    14, 10, 8, 4,
    3, 7, 9, 13, 17, 16,

    # 右侧两列: Col5向下, Col6向上到22
    12, 6, 2,
    1, 5, 11, 15, 22,
...
def init_grid(offset_x: float = ORIGIN_OFFSET_X,
    ...
    for (row, col), bid in _LAYOUT.items():
        x = offset_x + (GRID_ROWS - 1 - row) * BLOCK_SIZE
        y = offset_y + col * BLOCK_SIZE
